...
#!/usr/bin/env python3
"""
Price Uploader - Upload StockAnalysis backup prices to Supabase
Runtime: ~5 seconds
...
- Original: /opt/selenium_automation/price_uploader.py
"""
...
logger = setup_logging('stockanalysis_ohlcv_uploader', log_to_file=True)
...
class PriceUploader:
    ...
    def upload_prices(self, csv_file):
        """Upload prices to Supabase"""
        logger.info("=" * 70)
        logger.info("UPLOADING PRICES TO SUPABASE")
        logger.info("=" * 70)
    ...
    def run(self, csv_file):
        ...
        if success:
            logger.info("")
            logger.info("=" * 70)
            logger.info("PRICE UPLOAD COMPLETE!")
            logger.info("=" * 70)
            logger.info(f"Table: {self.table_name}")
            logger.info(f"Runtime: ~15 seconds")
            logger.info("=" * 70)
            return True
        else:
...
def main():
    ...
    try:
        ...
        try:
            ...
            if len(sys.argv) < 2:
                logger.error("Usage: python3 stockanalysis_ohlcv_uploader.py <csv_file>")
                logger.error("   OR: Run after stockanalysis_ohlcv_collector.py")
                sys.exit(1)
            csv_file = sys.argv[1]

        uploader = PriceUploader()
